...
class	SceneManager
{
	...
	function	OnSetup(scene)
	{
		...
		local	bg_texture = ResourceFactoryLoadTexture(g_factory, "ui/hud_bg.png")
		bg = UIAddSprite(ui, -1, bg_texture, 1280 - TextureGetWidth(bg_texture), 0, TextureGetWidth(bg_texture), TextureGetHeight(bg_texture))
		SpriteSetOpacity(bg, 0.75)

		local	texture = ResourceFactoryLoadTexture(g_factory, "ui/life_bar.png")
		life_bar = UIAddSprite(ui, -1, texture, 1280 - TextureGetWidth(bg_texture) + 14, 8 + 6, TextureGetWidth(texture), TextureGetHeight(texture))
		SpriteSetOpacity(life_bar, 0.5)

		local	life_label = Label(ui, TextureGetWidth(bg_texture), TextureGetWidth(texture), SpriteGetPosition(life_bar).x , SpriteGetPosition(life_bar).y)
		life_label.label = "LIFE"
		life_label.label_color = 0x000000ff
		life_label.font = "visitor1"
		life_label.font_size = 56
		life_label.refresh()
